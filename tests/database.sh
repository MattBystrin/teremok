...
room_types() {
	# Insert room type
	local id=$(psql --csv -c "insert into room_types(capacity, cost, label) values (1, 4, 'room for predators') returning id" | tail +2 | head -1)
	psql -c "insert into rooms(type) values (${id})"
	local value=$(psql --csv -c "select id from rooms where type=${id}" | tail +2)
...
clean_tasks() {
	...
	[[ -n $task ]] || return 1
	psql -c "delete from users where email = 'testemployee@mail.ru'"
	task=$(psql --csv -c "select id from clean_tasks where user_id = $user" | tail +2)
	[[ -z $task ]] || return 1
	return 0
}
...
for tcase in book_records room_types clean_tasks users
do
